...
createModel_DDexp_multi_geo <- function(tree,geo.object,r.object){    
        ...
        paramsNames <- c("m0", "logsigma0_1","logsigma0_2", "r1", "r2")
        params0 <- c(0,log(1),log(1),-0.1,-0.1)
        ...
        aAGamma <- function(i, params){
            ...
            matrixGamma <- function(t) return((exp(params[2])*1/colSums(rmat)*exp(params[4]/2*nij1)*diag(rmat[1,]))+(exp(params[3])*1/colSums(rmat)*exp(params[5]/2*nij2)*diag(rmat[2,])))
            matrixA <- diag(0, length(vectorU))
            return(list(a=vectorA, A=matrixA, Gamma=matrixGamma))
        }

		constraints <- function(params) return(params[3]<=Inf && params[3] >= -Inf && params[4]<=Inf && params[4]>=-Inf && params[2]<=Inf && params[2] >= -Inf && params[5]<=Inf && params[5]>=-Inf)
        model <- new(Class="PhenotypicADiag", name="DDexpM_geo", period=periodizing$periods, aAGamma=aAGamma, numbersCopy=eventEndOfPeriods$copy, numbersPaste=eventEndOfPeriods$paste, initialCondition=initialCondition, paramsNames=paramsNames, constraints=constraints, params0=params0, tipLabels=eventEndOfPeriods$labeling, comment=comment)

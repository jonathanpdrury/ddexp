...
source('fit_t_general5.2_BETA_JPD.R')
source('generalized_functions.R')
source('CreateGeobyClassObject_mvMORPH_BETA.R')
...
for(i in 1:2){
	...
	for(j in 5:11){
		...
		out<-CreateGeobyClassObject_mvMORPH(tan.tree,guild.mapi,ana.events=tanagers_RES_ana_events_tables_SMv2[[i]],clado.events=tanagers_RES_clado_events_tables_SMv2[[i]],trim.class="fruit")

##this version is meant to take a simmap object with letters as states, and a state that corresponds to two other states contains both letters
#e.g., AB is a lineage in both states A and B.

CreateGeoObject_overlap<-function(phylo,map){
			...
			for(j in 1:length(var.list)){
				for(k in 1:length(var.list)){
					if((lower.tri(int.mat,diag=TRUE)[j,k]==TRUE)){
						int.mat[j,k]<-ifelse(grepl(paste(strsplit(timei[match(var.list[j],timei[,1]),2],split="")[[1]],collapse="|"),timei[match(var.list[k],timei[,1]),2]),1,0)
					}
				}
			}
...
#							if(int.mat[j,k]==0){
#								if(mat[match(var.list[j],mat[,2]),4]==both || mat[match(var.list[k],mat[,2]),4]==both){
#								int.mat[j,k]<-1
#								}
#							}
#						}

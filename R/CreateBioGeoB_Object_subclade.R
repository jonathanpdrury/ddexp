...
##And builds a geo.object to be sent to fit_t_comp and similar functions
...
##v3 removes the mrca function call, which was crippling the script with larger trees
...
CreateBioGeoB_Object_subclade<-function(anc.phylo,subclade.phylo,ana.events,clado.events,nat.only=FALSE){
	...
	tips = clado.events$node[which(clado.events$label%chin%subclade.tips)]
	if(length(tips)>450){
		nn<-subclade.root

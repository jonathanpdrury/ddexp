...
sim_t_general <- function(tree, fun, beta=NULL, sigma=NULL, root=NULL, n.sim=1, model=c("exponential","linear")) {
...
phylo <- BranchtransformMAPS(phy=phy, beta, mtot, times, fun, sigma, model, errorValue=NULL)

#return(phylo)
	vcv.matrix=vcv(phylo)
	mvrnorm(n=n.sim,mu=rep(root,nrow(vcv.matrix)),Sigma=vcv.matrix)
